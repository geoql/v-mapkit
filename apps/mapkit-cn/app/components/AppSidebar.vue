...
<template>
  <Sidebar side="left" variant="sidebar" collapsible="offcanvas">
    <SidebarHeader class="border-b border-sidebar-border px-4 py-3">
      ...
      >
        <Icon name="lucide:map" class="size-5 text-primary" />
        <span>mapkit<span class="text-muted-foreground">-cn</span></span>
      </NuxtLink>
    </SidebarHeader>
    ...
    <SidebarContent class="hide-scrollbar">
      <nav aria-label="Example navigation">
        ...
        <SidebarGroup v-for="group in exampleGroups" :key="group.title">
          ...
          <SidebarMenu>
            ...
            >
              <SidebarMenuButton
                as-child
                size="sm"
                :is-active="route.path === `/examples/${example.slug}`"
              >
                <NuxtLink :to="`/examples/${example.slug}`">
